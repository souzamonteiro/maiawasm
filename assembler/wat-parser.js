class Lexer {
  ...
  tokenize() {
    while (this.position < this.input.length) {
      ...
      // Keep plain unsigned integers as `nat` so grammar rules for limits,
      // indices, memory pages, and other natural-number positions continue to
      // work, while still allowing decimal/exponent forms to be tokenized as
      // `float` for f32.const/f64.const immediates.
      if (candidates.length > 0 && bestPattern && bestPattern.type === 'float' && /^(?:[0-9]+)$/.test(bestMatch[0])) {
        const natCandidate = candidates.find((candidate) => candidate.pattern.type === 'nat' && candidate.match[0] === bestMatch[0]);
        if (natCandidate) {
          bestPattern = natCandidate.pattern;
          bestMatch = natCandidate.match;
        }
      }
